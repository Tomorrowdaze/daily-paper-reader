...
title_zh: 可验证的灾害叙事与因果知识图谱：以引文为依据的异构人道主义来源管线
...
tldr: 提出结合EM-DAT与ReliefWeb/EMM的RAG流水线，提取含17字段的灾害故事线与可溯源因果图；在三类危机、18名评估者人评中检索精度与因果忠实度高，专家偏好带引用组件；系统可扩展至全库并拟公开叙事实例化数据库。
source: arxiv
...
motivation: 人道应急早期信息量巨大且异构，人工难以及时综合以支撑态势感知与决策。
method: 采用检索增强生成，将EM-DAT结构化记录与ReliefWeb/EMM文本融合，生成17字段事件表与带引用叙事的因果知识图。
result: 人评显示检索精度高、因果关系忠实，且专家显著偏好具引用溯源的组件优于未溯源方案。
conclusion: 该可追溯、可扩展管线有望覆盖EM-DAT全库并公开发布叙事增强数据库，促进快速可靠的人道态势分析。
---

## 摘要
高效的人道主义响应依赖于对异构且海量的信息源进行快速综合——而在危机发生后的关键最初数小时内，这一任务常常超出人类的分析能力。我们提出一条管线，将 EM-DAT 的结构化灾害记录与 ReliefWeb 和 European Media Monitor（EMM）的非结构化文档相结合，生成以来源为依据的灾害叙事与因果知识图谱，以支持应对人员和分析师的态势感知。借助检索增强生成，该管线提取结构化叙事——表格化的事件画像，覆盖17个字段，从严重程度与关键驱动因素到儿童敏感型影响指标——并构建因果知识图谱，其中每个节点与边都配有以引文为依据的解释性叙述，从而可完全追溯至原始来源。我们在三个多样化的危机用例上对系统进行了评估，开展了包含9名领域专家和9名非专家评估者的人类评估。结果证实了较高的检索准确率、所提取因果关系的高度忠实性，以及专家对有引文支撑组件相较于无引文支撑替代方案的明确偏好。该管线旨在扩展至整个 EM-DAT 目录，目标是公开发布数据库的叙事增强版本。
