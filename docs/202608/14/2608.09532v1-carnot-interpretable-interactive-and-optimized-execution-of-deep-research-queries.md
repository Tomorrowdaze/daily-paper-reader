...
title_zh: Carnot：可解释、交互式且优化的深度研究查询执行
...
tags: ["query:q5"]
score: 6.0
evidence: 深度研究查询的可解释执行和中间推理透明度
tldr: Carnot提出一种面向企业数据湖的可解释、可交互、可优化的AI分析执行引擎，将自然语言请求编译为物理执行图，通过笔记本界面暴露中间过程，允许用户审阅与干预，并在成本或延迟约束下进行查询优化，实现高效可验证洞察。
source: arxiv
...
motivation: 企业用自然语言查询数据湖时，现有深度研究代理黑箱且缺乏成本/延迟控制，语义算子又成本高，导致难以验证与纠偏。
method: Carnot将自然语言编译成物理执行图，借助交互式笔记本界面暴露计划与中间数据，支持逐步执行、编辑代码/算子指令，并提供基于成本或延迟约束的查询优化器。
result: 系统使用户能在执行过程中审阅与修正推理与取数步骤，减少幻觉前提与资源浪费，获得高效且可验证的分析结果。
conclusion: Carnot为AI驱动分析提供了可解释、可交互且可控的执行路径，在真实企业工作负载中展示了效率与验证性的兼得。
---

## 摘要
企业越来越希望通过基于 AI 的工具（如语义算子或深度研究代理）使用自然语言查询数据湖。然而，后者以不透明的黑箱方式运行，隐藏其中间推理与数据检索步骤，且未暴露用于管理 API 成本和执行延迟的控制手段。与此同时，前者在企业规模的数据湖上成本可能高得难以承受。因此，使用这些系统的分析师缺乏干预能力，无法拦截幻觉式前提、验证中间结果，或纠正系统的执行轨迹。我们提出 Carnot，这是一款用于 AI 驱动分析的交互式执行引擎。Carnot 将自然语言请求编译为物理执行图，并通过交互式笔记本界面呈现。用户无需盲目等待最终输出，他们可以评审计划、增量执行算子、检查中间数据，或直接编辑底层代码或语义算子指令。Carnot 的查询优化器将依据用户提供的成本或延迟约束对查询进行优化。我们的演示将展示 Carnot 如何在源于真实企业用例的工作负载上，帮助用户获得高效且可验证的洞察。
